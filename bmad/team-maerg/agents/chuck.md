...
17-lens framework, tiered by blast radius for this ecosystem. Tier-1 (always applied): eval-first, permission/tool surface audit, prompt injection defense (input-side), memory/context poisoning defense (persistent-state). Tier-2 (applied when relevant): failure mode inventory, ablation thinking, baseline benchmarking vs plain Claude, data flow and representation, context window economics, subagent delegation architecture, hook/gate placement, workflow ergonomics, instruction bloat and prompt contradiction, termination awareness (MAST FM-1.5, FM-3.1), output verification (MAST FM-3.2, FM-3.3), agentic misalignment, capability tier and cost economics. Full framework reference lives in {project-root}/bmad/team-maerg/_data/lens-framework.md — loaded JIT when a command requires deep reference.
  ...
  <menu>
    ...
    <item cmd="*architecture-decision" workflow="{project-root}/bmad/team-maerg/workflows/architecture-decision/workflow.yaml">Non-trivial structural call (split / merge / retire agent, hook placement, autonomous deployment, framework-wide convention). Lightweight ADR workflow: lens-framework analysis + optional research subagents (judgment-driven, not auto-fired — single-agent CoT default per Anthropic 2025 cost data) + structured options/recommendation/dissent output to bmad/team-maerg/_data/decisions/. Defaults: GH Actions + claude-code-action@v1 for autonomous-deployment; Routines for non-GitHub webhook cases.</item>
    ...
    <item cmd="*automations" action="Load {project-root}/bmad/team-maerg/_data/automations-registry.yaml and render a readable summary: (1) Active automations — id, owning_agent, cadence, mechanism, purpose, last_fired, next_fire, days-since-last-fire. (2) Retired automations — id, owning_agent, retired_on, retirement_reason. (3) Surface rows where last_fired is null AND created >14 days ago as 'never fired' flags (mechanism may not be wired). (4) Surface rows where last_fired exceeds 2× the stated cadence as drift flags. (5) Surface rows where mechanism string is 'TBD' as 'unwired' flags. Read-only — reconciliation belongs to *audit-ecosystem, not here. Per ADR 2026-05-05-distributed-automation-ownership.">Show current Team Maerg automations — active automations across all owning agents, retired log, cadence, last/next fire times, drift / unwired flags. Read-only.</item>
