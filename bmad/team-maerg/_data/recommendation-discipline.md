...
- **Footer-as-investigation-substitute.** Applying full T2/T3 footer rigor to a question whose right answer is "go look first." When an action fails with a specific error, when a command emits a targeted diagnostic, when the user reports unexpected behavior — the FIRST move is to read the error / run the diagnostic / inspect the state. Footer fires AFTER investigation, not as a substitute for it. A structurally-perfect footer applied to the wrong question is theatre at the recommendation-level (vs the footer-content-level theatre that "Theatre footers" addresses). Symptom: pivoting from a specific error message to broad strategic framing ("should we change posture?", "what's our convention?") without first running the obvious diagnostic the error itself names.
